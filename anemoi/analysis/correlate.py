...
def return_correlation_data_from_masts(ref_mast, site_mast):
    ...
    if not valid_ws_correlation_data(data=data, ref_ws_col='ref', site_ws_col='site'):
        warning_string = "Warning: {} and {} don't seem to have valid concurrent data for a correlation.".format(
            ref_mast.name, site_mast.name)
...
def calculate_EDF_uncertainty(data, ref_ws_col='ref', site_ws_col='site'):
    """Calculate the EDF estimated correlation uncertainty between two wind speed columns.
    Assumes a correlation forced through the origin
    ...
    uncert = np.sqrt(
        (Xsi2 * SU ** 2) * (Sxx * Sxy ** 2 + 0.25 * ((Sxx - Syy) ** 2) * Sxx) / ((B ** 2 + 1.0) * Sxy ** 4))
    return uncert * 100.0
...
def ws_correlation_method(data, ref_ws_col='ref', site_ws_col='site', method='ODR', force_through_origin=False):
    ...
    """
    if method == 'ODR':
        results = ws_correlation_orthoginal_distance_model(data=data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col,
                                                           force_through_origin=force_through_origin)
    elif method == 'OLS':
        results = ws_correlation_least_squares_model(data=data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col,
                                                     force_through_origin=force_through_origin)
    elif method == 'RLM':
        results = ws_correlation_robust_linear_model(data=data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col,
                                                     force_through_origin=force_through_origin)

    return results


def ws_correlation_binned_by_direction(data, ref_ws_col='ref', site_ws_col='site', ref_dir_col='dir', dir_sectors=16,
                                       method='ODR', force_through_origin=False):
    ...
    for dir_bin in dir_bins:
        ...
        if not valid_ws_correlation_data(data=dir_bin_data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col):
            ...
            dir_bin_results = ws_correlation_method(data=dir_bin_data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col,
                                                    method=method, force_through_origin=force_through_origin)
            results.loc[dir_bin, ['slope', 'offset', 'R2', 'uncert', 'points']] = dir_bin_results.values

    return results


def ws_correlation_binned_by_month(data, ref_ws_col='ref', site_ws_col='site', method='ODR',
    ...
    for month in months:
        ...
        if not valid_ws_correlation_data(data=monthly_data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col):
            ...
            monthly_results = ws_correlation_method(data=monthly_data, ref_ws_col=ref_ws_col, site_ws_col=site_ws_col,
                                                    method=method, force_through_origin=force_through_origin)
...
def masts_10_minute(ref_mast, site_mast, ref_ws_col=None, site_ws_col=None, method='ODR', force_through_origin=False):
    ...
    valid_results = ws_correlation_method(data=data, ref_ws_col='ref', site_ws_col='site', method=method,
                                          force_through_origin=force_through_origin)
    results.loc[pd.IndexSlice[ref_mast.name, site_mast.name], ['slope', 'offset', 'R2', 'uncert',
                                                               'points']] = valid_results.values
    return results


def masts_10_minute_by_direction(ref_mast, site_mast, ref_ws_col=None, ref_dir_col=None, site_ws_col=None,
                                 site_dir_col=None, method='ODR', force_through_origin=False, dir_sectors=16):
    ...
    results = ws_correlation_binned_by_direction(data, dir_sectors=dir_sectors, method=method,
                                                 force_through_origin=force_through_origin)
...
def masts_daily(ref_mast, site_mast, ref_ws_col=None, site_ws_col=None, method='ODR', force_through_origin=False,
    ...
    ref_data_daily_mean = an.utils.mast_data.resample_mast_data(ref_ws_data, freq='daily',
                                                                minimum_recovery_rate=minimum_recovery_rate)
    site_data_daily_mean = an.utils.mast_data.resample_mast_data(site_ws_data, freq='daily',
                                                                 minimum_recovery_rate=minimum_recovery_rate)
...
def masts_daily_by_month(ref_mast, site_mast, ref_ws_col=None, site_ws_col=None, method='ODR',
                         force_through_origin=False, minimum_recovery_rate=0.7):
...
def apply_10min_results_by_direction(ref_mast, site_mast, corr_results, ref_ws_col=None, ref_dir_col=None,
                                     site_ws_col=None, splice=True):
...
def apply_daily_results_by_month_to_mast_data(mast_data, corr_results, ref_ws_col='ref', site_ws_col='site',
                                              splice=True):
