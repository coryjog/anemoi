...
class DataBase(object):
    ...
    def __init__(self, database=None, conn_str=None, conn=None, domino=False):
        ...
        if domino:
            user_id = os.environ.get('EDF_UID')
            password = os.environ.get('EDF_Password')
            conn_str = 'DRIVER={ODBC Driver 11 for SQL Server};SERVER=%s;DATABASE=%s;UID=%s;PWD=%s' %(server, db, user_id, password)
        else:
    ...
    def get_meter_data(self, project):
        if not self.connection_check('PadrePI'):
    ...
    def get_masts(self):
        ...
        masts.set_index(['Region', 'Project', 'AssetID', 'Type'], inplace=True)
    ...
    def get_sensor_data(self, vs=None, sensor_name=None, start_date=None, end_date=None, signal=1):
        ...
        WHERE MDVT_ID = %i and MVS_ID = %s 
        %s
        ORDER BY MRD_CorrectedTimestamp''' % (sensor_name, signal, str(vs), start_end_str)
    ...
    def get_mast_data(self, project=None, mast=None, start_date=None, end_date=None):
        ...
        mast: string, default None
            Name of the mast from which to download the data
        start_date: str, default None
            Date at which to start the data
            Assumed to be ISO format 'yyyy-mm-dd'
            If None, will begin at the begining of the measured period
        ...
        sensors = masts.loc[pd.IndexSlice[:, project, str(mast)],'MVS_ID']

        mast_data = pd.DataFrame()
        for sensor in sensors:
            name = masts.loc[masts.MVS_ID == sensor, 'Name'].values[0]
            data = self.get_sensor_data(sensor, name, start_date=start_date, end_date=end_date)
            mast_data = pd.concat([mast_data, data], axis=1)

        mast_data = mast_data[mast_data.columns.sort_values()]
        return mast_data
