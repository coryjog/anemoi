...
class M2D2(object):
    ...
    def get_wmm_id_data(self, wmm_id, start_date, end_date):
        ...
        Returns:
        ________
        DataFrame of measured virtual sensor data with sensor names as the column labels
        '''

        mast_metadata = self.get_masts().loc[pd.IndexSlice[:,wmm_id],:]
        sensors = mast_metadata.loc[:,['MVS_ID', 'Name']].set_index('MVS_ID')
